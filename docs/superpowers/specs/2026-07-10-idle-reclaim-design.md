...
- `async reapOnce(): Promise<void>` — 遍历 `this.channels`；对每个 `channel.isIdleReclaimable(Date.now(), this.config.idleReclaimMs)` 为真的 channel：
  ```
  if (channel.reinitializing) continue;             // 正被其它路径重建，跳过
  try {
    channel.reinitializing = true;                  // 让 Worker.getIdleChannel 跳过它（竞态守卫）
    await channel.close();
  ...
  ```
  注：置 `reinitializing=true` 后**不再复判** `isIdleReclaimable`——`Channel.isIdleReclaimable` 在 `reinitializing=true` 时恒为 `false`，复判是死代码且会令回收永不生效。竞态防护由 `reinitializing` 标志本身（`getIdleChannel` 跳过 `reinitializing` 的 channel）与置位前那一次 `isIdleReclaimable` 判断提供。
...
Reaper:    每 30s 扫描 → channel 空闲 > 5min 且 !busy && !reinitializing
           → reinitializing=true → channel.close() → reinitializing=false
...
- **reaper ↔ runTask**：reaper 以 `reinitializing=true` 包裹整个 `close()`（`getIdleChannel` 跳过 `reinitializing` 的 channel），空闲判定在置位前完成；`ensureContext()` 在 `busy=true` 之后执行，即便 page 刚被关也能重建——不丢任务。
