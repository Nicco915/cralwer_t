...
**架构：** 在 `Worker.runTask` 中，第一次 `channel.crawl` 返回 `not_found` 且 `error === 'Page shows no result'` 时，临时把 `task.baseUrl` 切换为 US URL 再 crawl 一次；`result.regionCode` 保持原请求区域。`RegionRegistry` 内置 US 已由 main 启用（commit `835741c`），本计划不再重复。
...
**注意：** `src/region-registry.js` 与 Windows `.env.example` 中的 US 启用已由 main（`835741c`）完成，本计划跳过。
...
```bash
node --test test/worker-region.test.js test/region-registry.test.js
```
...
### 任务 4：更新加拿大 Windows 部署配置

**文件：**
- 修改：`deployment/windows/ecosystem.canada.config.js`

**注意：** `scripts/deploy/windows/native/.env.example` 与 `docker/.env.example` 中的 US 启用示例已由 main（`835741c`）更新，本任务跳过。

- [ ] **步骤 1：在加拿大 PM2 配置中注入区域映射**
...
```bash
git add deployment/windows/ecosystem.canada.config.js
git commit -m "docs(deploy): 加拿大 Windows PM2 配置显式注入 US 区域"
```
...
- **规格覆盖度：**
  - 兜底规则 ✓（任务 3）
  - result.regionCode 保持原区域 ✓（任务 2 测试断言）
  - 全局超时 200s ✓（任务 1）
  - 加拿大配置更新 ✓（任务 4）
