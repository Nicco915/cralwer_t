...
function createClient(proxies, auth) {
  return {
    getKpsProxies: async () => proxies,
    getProxyAuthorization: async () => auth || { username: 'user', password: 'pass' },
  };
}
...
test('partitions proxies by machine index and assigns per channel', async () => {
  ...
  assert.ok(map['ch-1'].startsWith('http://user:pass@1.1.') || map['ch-1'].startsWith('http://user:pass@3.3.') || map['ch-1'].startsWith('http://user:pass@5.5.'));
  assert.ok(map['ch-2'].startsWith('http://user:pass@1.1.') || map['ch-2'].startsWith('http://user:pass@3.3.') || map['ch-2'].startsWith('http://user:pass@5.5.'));
  assert.notStrictEqual(map['ch-1'], map['ch-2']);

  const saved = JSON.parse(fs.readFileSync(assignmentsFile, 'utf-8'));
  assert.deepStrictEqual(Object.keys(saved).sort(), ['ch-1', 'ch-2']);
  assert.ok(saved['ch-1'].startsWith('1.1.') || saved['ch-1'].startsWith('3.3.') || saved['ch-1'].startsWith('5.5.'));
  assert.ok(!saved['ch-1'].includes('user'));
...
test('reuses previous assignment when IP is still in partition', async () => {
  ...
  const map = await pool.assign();
  assert.strictEqual(map['ch-1'], 'http://user:pass@2.2.2.2:8080');
  try { fs.unlinkSync(assignmentsFile); } catch (e) {}
});

test('refresh reports changed channels', async () => {
  const assignmentsFile = path.join(os.tmpdir(), `pool-${Date.now()}.json`);
  const client = createClient(['1.1.1.1:8080', '2.2.2.2:8080']);
  ...
  pool.client = createClient(['3.3.3.3:8080', '4.4.4.4:8080']);
...
test('throws when partition is smaller than channel count', async () => {
  const client = createClient(['1.1.1.1:8080']);
...
test('falls back to ip:port when proxy auth is unavailable', async () => {
  const assignmentsFile = path.join(os.tmpdir(), `pool-${Date.now()}.json`);
  const client = {
    getKpsProxies: async () => ['1.1.1.1:8080'],
    getProxyAuthorization: async () => { throw new Error('auth not supported') },
  };
  const pool = new ProxyPool({ client, machineIndex: 0, machineTotal: 1, channels: 1, assignmentsFile });
  const map = await pool.assign();
  assert.strictEqual(map['ch-1'], 'http://1.1.1.1:8080');
  try { fs.unlinkSync(assignmentsFile); } catch (e) {}
});
